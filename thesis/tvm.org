...
Model przetwarzania współbieżnego został już przybliżony przy okazji ogólnego opisu architektury ThesisVM na początku rozdziału. Wybrany został model SMP - symetryczny multiprocesor, którego cechą szczególną jest istnienie wielu identycznych jednostek operacyjnych, wykonujących jednakowe operacje na różnych zbiorach danych (rysunek [[ref:fig:amp-vs-smp]]).
...
Alternatywnym rozwiązaniem jest model asymetrycznego multiprocessingu (rysunek [[ref:fig:amp-vs-smp]]), gdzie dla różnych typów zadań istnieją dedykowane jednostki operacyjne, takie jak wątki, czy procesy systemu operacyjnego. Rozwiązania asymetryczne są interesujące ze względu na zupełnie nowe klasy algorytmów zarządzania pamięcią, których implementację umożliwiają (na przykład algorytm VCGC [[cite:Huelsbergen1998]] wykorzystujący trzy asymetryczne wątki).
